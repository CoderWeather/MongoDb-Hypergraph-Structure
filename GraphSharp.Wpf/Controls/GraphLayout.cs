...
namespace GraphSharp.Controls
{
	...
	{
		protected readonly Dictionary<TEdge, EdgeControl> EdgeControls;
		private readonly Queue<TEdge> _edgesAdded;
		private readonly Queue<TEdge> _edgesRemoved;
		private readonly List<LayoutState<TVertex, TEdge>> _layoutStates;
		private readonly TimeSpan _notificationLayoutDelay; // 5 ms
		private readonly object _notificationSyncRoot;

		protected readonly Dictionary<TVertex, VertexControl> VertexControls;

		private readonly Queue<TVertex> _verticesAdded;
		private readonly Queue<TVertex> _verticesRemoved;
		private readonly Stopwatch _stopWatch;
		private DateTime _lastNotificationTimestamp;
		...
		protected IDictionary<TVertex, SizeF> VertexSizes
		{
			get
			{
				...
				foreach (var (vertex, vertexControl) in VertexControls)
				{
					var size = vertexControl.DesiredSize;
					Sizes.Add(vertex, new SizeF(
		...
		protected LayoutMode ActualLayoutMode
		{
			get
			{
				if (UiLayoutMode == UiLayoutMode.Compound ||
					UiLayoutMode == UiLayoutMode.Automatic && Graph != null && Graph is ICompoundGraph<TVertex, TEdge>)
		...
		protected bool IsCompoundMode => ActualLayoutMode == LayoutMode.Compound;
			...
			IDictionary<TVertex, Point>? positions, IDictionary<TVertex, Size> sizes)
		{
			...
			var borders = VertexControls
			   .Where(kvp => kvp.Value is CompoundVertexControl)
			   .ToDictionary(
					kvp => kvp.Key,
					kvp => ((CompoundVertexControl) kvp.Value).VertexBorderThickness);

			var layoutTypes = VertexControls
			   .Where(kvp => kvp.Value is CompoundVertexControl)
			   .ToDictionary(
					kvp => kvp.Key,
		...
		protected virtual void Layout(bool continueLayout)
		{
			if (Graph == null || Graph.VertexCount == 0 ||
				!LayoutAlgorithmFactory.IsValidAlgorithm(LayoutAlgorithmType) || !CanLayout)
		...
		private IDictionary<TVertex, Point> GetLatestVertexPositions()
		{
			...
			else
			{
				...
				foreach (var (vertex, vertexControl) in VertexControls)
				{
					var position = vertexControl.TranslatePoint(topLeft, this);
					position.X += vertexControl.ActualWidth / 2;
					position.Y += vertexControl.ActualHeight / 2;
		...
		private IDictionary<TVertex, Point> GetRelativePositions(bool continueLayout)
		{
			...
			if (continueLayout)
				foreach (var (vertex, vertexControl) in VertexControls)
					posDict[vertex] = GetRelativePosition(vertexControl);
			else
				foreach (var (_, vertexControl) in VertexControls)
				{
					if (!(vertexControl is CompoundVertexControl))
						continue;

					var cvc = (CompoundVertexControl) vertexControl;
		...
		private IDictionary<TVertex, Size> GetLatestVertexSizes()
		{
			...
			foreach (var (vertex, vertexControl) in VertexControls)
				vertexSizes[vertex] = new Size(vertexControl.ActualWidth, vertexControl.ActualHeight);
		...
		{
			if (iterArgs?.VertexPositions == null)
			{
				if (LayoutAlgorithm is ICompoundLayoutAlgorithm<TVertex, TEdge, TGraph> algorithm)
				{
		...
		protected virtual void OnLayoutIterationFinished(
			IDictionary<TVertex, Point> vertexPositions, string message)
		{
			...
			IDictionary<TVertex, Size> sizes)
		{
			...
			foreach (var (vertex, rect) in OverlapRemovalAlgorithm.Rectangles)
				result[vertex] = new Point(
					rect.Left + rect.Size.Width * 0.5,
					rect.Top + rect.Size.Height * 0.5);
		...
		protected void ChangeState(int stateIndex)
		{
			...
			if (positions != null)
				...
				{
					if (!VertexControls.TryGetValue(v, out var vp))
						continue;

					if (positions.TryGetValue(v, out var pos))
			...
			if (activeState.RouteInfos != null)
				...
				{
					if (!EdgeControls.TryGetValue(e, out var ec))
						continue;

					activeState.RouteInfos.TryGetValue(e, out var routePoints);
