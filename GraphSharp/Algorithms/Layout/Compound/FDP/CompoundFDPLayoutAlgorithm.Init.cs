...
namespace GraphSharp.Algorithms.Layout.Compound.FDP
{
		...
		where TGraph : IBidirectionalGraph<TVertex, TEdge>
	{
		...
		{
			foreach (var e in elements) set.Remove(e);
		}
        ...
        ///     </list>
        /// </summary>
        /// <param name="vertexSizes">
        ///     The dictionary of the inner canvas sizes
        ///     of the compound vertices.
        /// </param>
        /// <param name="vertexBorders">
        ///     The dictionary of the border thickness of
        ///     the compound vertices.
        /// </param>
        /// <param name="layoutTypes">
        ///     The dictionary of the layout types of
        ///     the compound vertices.
        /// </param>
        ...
        ///     The compound vertices with fixed layout
			...
			Queue<TVertex> movableParentUpdateQueue)
		{
			for (var i = Levels.Count - 1; i >= 0; i--)
				foreach (var vertex in Levels[i])
				{
					...
					if (layoutType == CompoundVertexInnerLayoutType.Fixed) movableParentUpdateQueue.Enqueue(vertex);

					var position = new Point();
					VertexPositions.TryGetValue(vertex, out position);

					//create the information container for this compound vertex
					var dataContainer = new CompoundVertexData(vertex, _rootCompoundVertex, false, position, vertexSize,
						border, layoutType);
					if (i == 0)
					{
						dataContainer.Parent = _rootCompoundVertex;
		...
		private void InitialLevels()
		{
			...
			//initial 0th level
			Levels.Add(
				new HashSet<TVertex>(VisitedGraph.Vertices.Where(v =>
					_compoundGraph.GetParent(v) == default(TVertex))));
