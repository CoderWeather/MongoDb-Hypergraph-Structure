...
namespace GraphSharp.Algorithms.Layout.Compound.FDP
{
		...
		where TGraph : IBidirectionalGraph<TVertex, TEdge>
	{
        ...
        protected override void InternalCompute()
		{
			...
			for (_phase = 1; _phase <= 3; _phase++)
			{
				...
				for (_step = _maxIterationCounts[_phase - 1];
					_step > 0 && true || _phase == 2 && !_allTreesGrown;
					_step--)
				{
					/*error = 0;*/
        ...
        private void ApplySpringForces()
		{
			...
			{
				if (!_allTreesGrown && (_removedRootTreeNodes.Contains(edge.Source) ||
					_removedRootTreeNodes.Contains(edge.Target)))
					continue;
				...
				else if (u.MovableParent != null) u.MovableParent.SpringForce += Fs;
				if (!v.IsFixedToParent)
					v.SpringForce -= Fs /* * v.Mass / (u.Mass + v.Mass)*/;
				else if (v.MovableParent != null) v.MovableParent.SpringForce -= Fs;
			}
		}
			...
			for (var i = Levels.Count - 1; i >= 0; i--)
			{
				...
				foreach (var uVertex in Levels[i])
				{
					...
					foreach (var vVertex in Levels[i])
					{
						...
						var Fr = GetRepulsionForce(u.Position, v.Position, u.Size, v.Size, repulsionRange) *
							Math.Pow(u.Level + 1, 2);
        ...
        private void ApplyGravitationForces()
		{
			for (var i = Levels.Count - 1; i >= 0; i--)
				foreach (var uVertex in Levels[i])
				{
					...
					Fg.Normalize();
					Fg *= Parameters.GravitationFactor * _gravityForceMagnitude * Math.Pow(u.Level + 1, 2) /
						Math.Pow(length, 0.25);
		...
		private void CalcNodePositionsAndSizes()
		{
			for (var i = Levels.Count - 1; i >= 0; i--)
				...
				{
					var u = _vertexDatas[uVertex];
					var force = u.ApplyForce(_temperature * Math.Max(1, _step) / 100.0 *
						Parameters.DisplacementLimitMultiplier);
				}
		}
