...
{
	public class
		ContextualLayoutAlgorithmFactory<TVertex, TEdge, TGraph> : IContextualLayoutAlgorithmFactory<TVertex, TEdge,
			TGraph>
		where TVertex : class
		...
		where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
	{
			...
			ILayoutContext<TVertex, TEdge, TGraph> context, ILayoutParameters parameters)
		{
			...
			{
				case "DoubleTree":
					return new DoubleTreeLayoutAlgorithm<TVertex, TEdge, TGraph>(layoutContext.Graph,
						layoutContext.Positions, layoutContext.Sizes, parameters as DoubleTreeLayoutParameters,
						layoutContext.SelectedVertex);
				case "BalloonTree":
					return new BalloonTreeLayoutAlgorithm<TVertex, TEdge, TGraph>(layoutContext.Graph,
						layoutContext.Positions, layoutContext.Sizes, parameters as BalloonTreeLayoutParameters,
						layoutContext.SelectedVertex);
				default:
					return null;
			}
		...
		public ILayoutParameters CreateParameters(string algorithmType, ILayoutParameters oldParameters)
		{
			...
			{
				case "DoubleTree":
					return !(oldParameters is DoubleTreeLayoutParameters)
						? new DoubleTreeLayoutParameters()
						: (DoubleTreeLayoutParameters) (oldParameters as DoubleTreeLayoutParameters).Clone();
				case "BaloonTree":
					return !(oldParameters is BalloonTreeLayoutParameters)
						? new BalloonTreeLayoutParameters()
						: (BalloonTreeLayoutParameters) (oldParameters as BalloonTreeLayoutParameters).Clone();
				default:
					return null;
			}
		}
