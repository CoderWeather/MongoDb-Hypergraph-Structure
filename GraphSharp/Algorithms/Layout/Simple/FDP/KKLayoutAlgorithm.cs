...
{
	public class
		KKLayoutAlgorithm<Vertex, Edge, Graph> : DefaultParameterizedLayoutAlgorithmBase<Vertex, Edge, Graph,
			KKLayoutParameters>
		where Vertex : class
		...
		where Graph : IBidirectionalGraph<Vertex, Edge>
	{
		...
		protected void Report(int currentIteration)
		{
			...
			//pozнciуk бtmбsolбsa a VertexPositions-ba
			for (var i = 0; i < vertices.Length; i++)
				VertexPositions[vertices[i]] = positions[i];

			#endregion

			OnIterationEnded(currentIteration, currentIteration / (double) Parameters.MaxIterations,
				"Iteration " + currentIteration + " finished.", true);
		}
        ...
        private double CalcEnergyIfExchanged(int p, int q)
		{
			...
			for (var j = i + 1; j < vertices.Length; j++)
			{
				...
				var l_ij = edgeLengths[i, j];
				var k_ij = springConstants[i, j];
				var dx = positions[ii].X - positions[jj].X;
				var dy = positions[ii].Y - positions[jj].Y;
