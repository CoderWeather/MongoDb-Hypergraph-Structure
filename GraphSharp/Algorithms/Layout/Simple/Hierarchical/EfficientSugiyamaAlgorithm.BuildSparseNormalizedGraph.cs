...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
		...
		where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
	{
		...
		private void RewriteLayerIndexes()
		{
			...
			{
				foreach (var vertex in layer) vertex.LayerIndex = i;
				i++;
			}
		}
		...
		private void CreateLayerWHOptInfos()
		{
			...
			foreach (var layer in _layers)
			{
				...
				foreach (var v in layerInfo.Vertices)
					if (!double.IsNaN(v.ValuePerCost) && !double.IsPositiveInfinity(v.ValuePerCost) &&
						!double.IsNegativeInfinity(v.ValuePerCost))
						vertexList.Add(v);
				vertexList.Sort((v1, v2) => Math.Sign(v2.ValuePerCost - v1.ValuePerCost));
				_actualHeight += layerInfo.LayerHeight + Parameters.LayerDistance;
				layerInfo.Vertices.Clear();
				foreach (var v in vertexList) layerInfo.Vertices.Enqueue(v);
		...
		private void CreateVertexWHOptInfos()
		{
			...
			foreach (var vertex in _graph.Vertices)
			{
				...
				whOptInfo.Value = vertex.Size.Width -
					Math.Max(0, _graph.InDegree(vertex) - 1) * Parameters.VertexDistance;
				whOptInfo.Cost = vertex.Size.Width -
					Math.Max(0, _graph.OutDegree(vertex) - 1) * Parameters.VertexDistance;
        ...
        private void CreateDummyVerticesAndSegments()
		{
			foreach (var edge in _graph.Edges.ToList())
			{
				...
				_graph.RemoveEdge(edge);
				var notReversed = edge.Source.OriginalVertex == edge.OriginalEdge.Source &&
					edge.Target.OriginalVertex == edge.OriginalEdge.Target;
        ...
        /// <param name="edge">
        ///     The edge which has been replaced by the
        ///     dummy vertices and this segment.
        /// </param>
        ...
        /// <returns>
        ///     The new vertex which has been added to the graph and the
        ///     layers.
        /// </returns>
