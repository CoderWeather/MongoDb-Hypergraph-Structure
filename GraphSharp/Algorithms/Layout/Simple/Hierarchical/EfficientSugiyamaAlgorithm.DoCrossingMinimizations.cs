...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
		...
		where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
	{
        ...
        private int Sweeping(int startLayerIndex, int endLayerIndex, int step, bool enableSameMeasureOptimization,
			out bool changed, ref int phase)
		{
			var crossings = 0;
			changed = false;
			...
			for (var i = startLayerIndex; i != endLayerIndex; i += step)
			{
				...
				if (_alternatingLayers[i + step] != null)
				{
					...
					prevCrossCount = DoCrossCountingAndOptimization(alternatingLayer, _alternatingLayers[i + step],
						i < i + step, false, phase == 2, int.MaxValue);
					_crossCounts[ci] = prevCrossCount;
				}

				var crossCount = CrossingMinimizationBetweenLayers(ref alternatingLayer, i, i + step,
					enableSameMeasureOptimization, prevCrossCount, phase);
		...
		private void OutputAlternatingLayer(AlternatingLayer alternatingLayer, int layerIndex, int crossCount)
		{
			...
			for (var i = 0; i < alternatingLayer.Count; i++)
				...
				else
				{
					...
					for (var j = 0; j < segmentContainer.Count; j++) Debug.Write("| \t");
				}
			...
			int phase)
		{
			...
			nextAlternatingLayer =
				InitialOrderingOfNextLayer(nextAlternatingLayer, _layers[nextLayerIndex], straightSweep);
			...
			/* 5 */
			var crossCount = DoCrossCountingAndOptimization(alternatingLayer, nextAlternatingLayer, straightSweep,
				enableSameMeasureOptimization, phase == 2, prevCrossCount);
		...
		private IList<SugiVertex> FindVerticesWithSameMeasure(AlternatingLayer nextAlternatingLayer, bool straightSweep,
			out IList<int> ranges, out int maxRangeLength)
		{
			...
			for (startIndex = 0; startIndex < vertices.Length; startIndex = endIndex + 1)
			{
				for (endIndex = startIndex + 1;
					...
					vertices[startIndex].MeasuredPosition == vertices[endIndex].MeasuredPosition;
					endIndex++)
				{
				}
			...
			int prevCrossCount)
		{
			...
			firstLayerSize = lastOnTopLayer.Position +
				(lastOnTopLayer is ISegmentContainer ? ((ISegmentContainer) lastOnTopLayer).Count : 1);
			secondLayerSize = lastOnBottomLayer.Position + (lastOnBottomLayer is ISegmentContainer
				? ((ISegmentContainer) lastOnBottomLayer).Count
				: 1);
			...
			if (enableSameMeasureOptimization || reverseVerticesWithSameMeasure)
			{
				IList<int> ranges;
				int maxRangeLength;
				var verticesWithSameMeasure = FindVerticesWithSameMeasure(nextAlternatingLayer, straightSweep,
					out ranges, out maxRangeLength);
				...
				else
				{
					...
					{
						if (stack.Count > 0 && (stack.Peek().MeasuredPosition != v.MeasuredPosition ||
							rnd.NextDouble() > 0.8))
							while (stack.Count > 0)
					...
					while (stack.Count > 0) sortedVertexList.Add(stack.Pop());
				}

				var maxPermutations = EfficientSugiyamaLayoutParameters.MaxPermutations;
				do
				{
					maxPermutations--;
					if (!reverseVerticesWithSameMeasure)
		...
		private bool PermutateRandom(IList<SugiVertex> vertices, int startIndex, int count)
		{
			...
			for (var i = startIndex; i < endIndex; i++) vertices[i].PermutationIndex = _rnd.Next(count);
			return true;
		}
		...
		private static int BiLayerCrossCount(IEnumerable<CrossCounterPair> pairs, int firstLayerVertexCount,
						...
						if (index % 2 > 0)
						{
							...
							switch (pair.Type)
							{
								case EdgeTypes.InnerSegment:
									...
									while (queue.Count > 0) queue.Dequeue().Marked = true;
									break;
								case EdgeTypes.NonInnerSegment:
									if (tree[index + 1].InnerSegmentMarker) pair.NonInnerSegment.Marked = true;
									break;
							}
						}
		...
		private static Queue<VertexGroup> GetContainerLikeItems(AlternatingLayer alternatingLayer,
		...
		private AlternatingLayer InitialOrderingOfNextLayer(AlternatingLayer alternatingLayer,
			IList<SugiVertex> nextLayer, bool straightSweep)
		{
			//get the list of the containers and vertices
			var segmentContainerStack =
				new Stack<ISegmentContainer>(alternatingLayer.OfType<ISegmentContainer>().Reverse());
			var ignorableVertexType = straightSweep ? VertexTypes.QVertex : VertexTypes.PVertex;
			var vertexStack = new Stack<SugiVertex>(nextLayer.Where(v => v.Type != ignorableVertexType)
			   .OrderBy(v => v.MeasuredPosition).Reverse());
