...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
		...
		where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
	{
		...
		private void DoOrthogonalEdgeRouting()
		{
			foreach (var edge in VisitedGraph.Edges)
			{
				...
				orthoRoutePoints[sourceIndex] = new Point
				{
					...
					Y = _layerPositions[sourceVertex.LayerIndex] + _layerHeights[sourceVertex.LayerIndex] +
				...
				EdgeRoutes[edge] = orthoRoutePoints;
			}

			foreach (var kvp in _dummyVerticesOfEdges)
			{
				var orthoRoutePoints = EdgeRoutes[kvp.Key];
				...
				routePoints[kvp.Value.Count + 2] =
					new Point(routePoints[kvp.Value.Count + 1].X, routePoints[kvp.Value.Count + 3].Y);
		...
		private void SavePositions()
		{
			foreach (var vertex in _graph.Vertices)
				if (vertex.Type == VertexTypes.Original)
					VertexPositions[vertex.OriginalVertex] =
						new Point(vertex.HorizontalPosition, vertex.VerticalPosition);
		}
		...
		private void PutbackIsolatedVertices()
		{
			...
			foreach (var isolatedVertex in _isolatedVertices)
			{
				...
				if (layer > 0 && prevIsolatedVertex != null)
					_graph.AddEdge(new SugiEdge(default, prevIsolatedVertex, isolatedVertex));
		...
		private void CalculateRealPositions()
		{
			foreach (var vertex in _graph.Vertices)
			{
				Debug.WriteLine("{0}:\t{1}\t{2}\t{3}\t{4}", vertex.OriginalVertex, vertex.HorizontalPositions[0],
					vertex.HorizontalPositions[1], vertex.HorizontalPositions[2], vertex.HorizontalPositions[3]);
		...
		{
			foreach (var vertex in _graph.Vertices)
				vertex.VerticalPosition = _layerPositions[vertex.LayerIndex] +
					(vertex.Size.Height <= 0 ? _layerHeights[vertex.LayerIndex] : vertex.Size.Height) / 2.0;
		}
		...
		{
			Debug.WriteLine("Alignment for {0}", modeIndex);
			foreach (var vertex in _graph.Vertices)
				Debug.WriteLine("{0},{1},{2}: Root {3},{4},{5}\tAlign {6},{7},{8}", vertex.OriginalVertex,
					vertex.LayerIndex, vertex.Type, vertex.Roots[modeIndex].OriginalVertex,
					vertex.Roots[modeIndex].LayerIndex, vertex.Roots[modeIndex].Type,
					vertex.Aligns[modeIndex].OriginalVertex, vertex.Aligns[modeIndex].LayerIndex,
					 ...
					 || leftRightMode == LeftRightMode.Right && _sparseCompactionGraph.OutDegree(w) > 0))
				{
					...
					foreach (var edge in edges)
					{
						...
						else
						{
							...
							u = upperLowerEdges == UpperLowerEdges.Upper
								? segment.PVertex.Roots[modeIndex]
								: segment.QVertex.Roots[modeIndex];
						}
						...
						var xDelta = delta + ((wVertex != null ? wVertex.Size.Width : 0.0) +
							(pred is SugiVertex ? ((SugiVertex) pred).Size.Width : u.BlockWidths[modeIndex])) / 2.0;
						...
						else
						{
							v.HorizontalPositions[modeIndex] =
								leftRightMode == LeftRightMode.Left
									? Math.Max(v.HorizontalPositions[modeIndex],
										u.HorizontalPositions[modeIndex] + xDelta)
									: Math.Min(v.HorizontalPositions[modeIndex],
										u.HorizontalPositions[modeIndex] - xDelta);
						}
					}
				}
				...
				for (var j = vertexStart; j != vertexEnd; j += vertexStep)
				{
					 ...
					 || vertex.Type == VertexTypes.QVertex && upperLowerEdges == UpperLowerEdges.Lower)
					{
						...
						for (var m = 0; m < medians.Length; m++)
						{
							...
							if (edge.Marked)
								Debug.WriteLine("Edge marked: " + edge.Source.OriginalVertex + ", " +
									edge.Target.OriginalVertex);
								 ...
								 || leftRightMode == LeftRightMode.Right && r > neighbour.Position))
							{
								...
								neighbour.BlockWidths[modeIndex] =
									Math.Max(neighbour.BlockWidths[modeIndex], vertex.Size.Width);
