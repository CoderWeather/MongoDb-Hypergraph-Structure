		...
		private class VertexLayer : List<SugiVertex>
		{
			...
			public void CalculateSubPriorities()
			{
				var orderedVertices = (from v in this
					orderby v.Priority, v.Measure, v.Position
					select v).ToArray();
			...
			public IEnumerable<SugiEdge> UpEdges
			{
				get
				{
					...
					foreach (var e in Graph.InHierarchicalEdges(v))
						yield return e;
				}
			}
			...
			public IEnumerable<SugiEdge> DownEdges
			{
				...
				{
					foreach (var v in this)
					foreach (var e in Graph.OutHierarchicalEdges(v))
						yield return e;
				}
			}
			...
			private static int CalculateCrossings(IEnumerable<SugiEdge> edges, bool sourcesByMeasure,
			...
			/// <param name="byRealPosition">
			///     If true, the barycenters will be computed based on the RealPosition.X value of the
			///     vertices. Otherwise the barycenter will be computed based on the value of the Position field (which is basically
			///     the index of the vertex inside the layer).
			/// </param>
			public void Measure(BaryCenter baryCenters, bool byRealPosition)
			{
				...
				foreach (var vertex in this)
					Measure(vertex, computeUpBaryCenter, computeDownBaryCenter, computeSubBaryCenter, divCount,
						byRealPosition);
			}
			...
			private void Measure(SugiVertex vertex, bool computeUpBaryCenter, bool computeDownBaryCenter,
				bool computeSubBaryCenter, int divCount, bool byRealPosition)
			{
				vertex.Measure = 0;
			...
			/// <param name="byRealPosition"></param>
			/// <returns>
			///     Returns with true if the vertices in this
			///     layer ordered by the given <paramref name="baryCenters" />.
			/// </returns>
			...
			/// <returns>
			///     Returns true if the vertices have permutated,
			///     otherwise (no more permutation) returns with false.
			/// </returns>
