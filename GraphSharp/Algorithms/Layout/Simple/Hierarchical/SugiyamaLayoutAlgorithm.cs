...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
		...
		where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
	{
		...
		*/
		...
		protected static void FilterParallelEdges<TVertexType, TEdgeType>(
			ISoftMutableGraph<TVertexType, TEdgeType> graph)
		...
		protected static void FilterIsolatedVertices<TVertexType, TEdgeType>(
			ISoftMutableGraph<TVertexType, TEdgeType> graph)
			where TEdgeType : class, IEdge<TVertexType>
		{
		...
		protected void PrepareForSugiyama()
		{
			...
			_graph.UnhideEdgesIf(e =>
				e.Type == EdgeTypes.General && _graph.ContainsVertex(e.Source) && _graph.ContainsVertex(e.Target));
			_graph.EdgeUnhidden -= eeh;
			...
			_graph.HideEdgesIf(e => e.Type == EdgeTypes.General && e.Source.LayerIndex != e.Target.LayerIndex,
        ...
        /// <returns></returns>
        protected bool SugiyamaPhase1Sweep(int start, int end, int step, BaryCenter baryCenter, bool dirty,
			bool byRealPosition)
		{
		...
		protected bool SugiyamaPhase1(int startLayerIndex, BaryCenter startBaryCentering, bool ByRealPosition)
		{
			...
			{
				sweepDownOptimized = SugiyamaPhase1Sweep(startLayerIndex == -1 ? 1 : startLayerIndex, _layers.Count, 1,
					BaryCenter.Up, dirty, ByRealPosition);
				startLayerIndex = -1;
			}

			var sweepUpOptimized = SugiyamaPhase1Sweep(startLayerIndex == -1 ? _layers.Count - 2 : startLayerIndex, -1,
				-1, BaryCenter.Down, dirty, ByRealPosition);
		...
		{
			var baryCenteringByRealPositions =
			...
			while (changed && (iteration1Left > 0 || iteration2Left > 0))
			{
				...
				//
				while (iteration1Left > 0 &&
					SugiyamaPhase1(startLayerIndex, startBaryCentering, baryCenteringByRealPositions))
				{
					iteration1Left--;
					changed = true;
				}
			...
			foreach (var e in _graph.GeneralEdges)
			{
				...
				var shouldBeMarked = e.Source.LayerIndex == e.Target.LayerIndex &&
		...
		{
			return Math.Max(0,
				(b.Size.Width + a.Size.Width) * 0.5 + plusGap + Parameters.HorizontalGap -
				(b.RealPosition.X - a.RealPosition.X));
		}
		...
		protected void HorizontalPositionAssignmentOnLayer(int layerIndex, BaryCenter baryCenter)
		{
			...
			foreach (var v in from vertex in layer
				orderby vertex.Priority, vertex.SubPriority
				select vertex)
			{
				...
				var v1 = v;
				var alreadySetVertices = layer.Where(vertex => !double.IsNaN(vertex.RealPosition.X) && vertex != v1)
				   .ToArray();
