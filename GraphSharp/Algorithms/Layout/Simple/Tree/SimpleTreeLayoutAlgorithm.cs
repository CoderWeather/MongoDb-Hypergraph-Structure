...
namespace GraphSharp.Algorithms.Layout.Simple.Tree
{
	...
	{
		private readonly IDictionary<TVertex, VertexData> data = new Dictionary<TVertex, VertexData>();
		private readonly IList<Layer> layers = new List<Layer>();
		private readonly IDictionary<TVertex, Size> sizes;
		private int direction;
		private BidirectionalGraph<TVertex, Edge<TVertex>> spanningTree;

		public SimpleTreeLayoutAlgorithm(TGraph visitedGraph, IDictionary<TVertex, Point> vertexPositions,
			IDictionary<TVertex, Size> vertexSizes, SimpleTreeLayoutParameters parameters)
		...
		private void GenerateSpanningTree()
		{
			...
			switch (Parameters.SpanningTreeGeneration)
			{
				case SpanningTreeGeneration.BFS:
					var bfsAlgo = new BreadthFirstSearchAlgorithm<TVertex, TEdge>(VisitedGraph, vb,
		...
		protected double CalculatePosition(TVertex v, TVertex parent, int l)
		{
			...
			else
			{
				...
				{
					var childPos = CalculatePosition(child, v, l + 1);
					if (childPos >= 0)
					{
