...
namespace GraphSharp
{
	public static class GraphHelper
	{
        ...
        public static IEnumerable<TEdge> GetEdgesBetween<TVertex, TEdge>(this IVertexAndEdgeListGraph<TVertex, TEdge> g,
			List<TVertex> set1, List<TVertex> set2)
			where TEdge : IEdge<TVertex>
		{
			...
			where Graph : IBidirectionalGraph<Vertex, Edge>
		{
			...
			foreach (var edge in undirected.Edges) weights[edge] = 1;
			...
			foreach (var source in g.Vertices)
			{
				...
				var spaDijkstra =
					new UndirectedDijkstraShortestPathAlgorithm<Vertex, Edge>(undirected, edge => weights[edge],
