...
namespace GraphSharp
{
		...
		where TEdge : IEdge<TVertex>
	{
		...
		private readonly IDictionary<string, HiddenCollection> hiddenCollections =
			new Dictionary<string, HiddenCollection>();
		...
		public IEnumerable<TVertex> HiddenVertices => hiddenVertices;

		public IEnumerable<TEdge> HiddenEdges => hiddenEdges;
		...
		public void HideVertices(IEnumerable<TVertex> vertices)
		{
			...
			foreach (var v in verticesToHide) HideVertex(v);
		}
		...
		{
			foreach (var v in vertices) HideVertex(v, tag);
		}
		...
		public void HideEdges(IEnumerable<TEdge> edges)
		{
			...
			foreach (var e in edgesToHide) HideEdge(e);
		}

		public void HideEdges(IEnumerable<TEdge> edges, string tag)
		{
			var edgesToHide = new List<TEdge>(edges);
			foreach (var e in edgesToHide) HideEdge(e, tag);
		}
		...
		public void UnhideEdges(IEnumerable<TEdge> edges)
		{
			...
			foreach (var e in edgesToUnhide) UnhideEdge(e);
		}

		public bool Unhide(string tag)
		{
			var h = GetHiddenCollection(tag);
			foreach (var v in h.hiddenVertices) UnhideVertex(v);
			foreach (var e in h.hiddenEdges) UnhideEdge(e);
			return hiddenCollections.Remove(tag);
		}

		public bool UnhideAll()
		{
			while (hiddenVertices.Count > 0) UnhideVertex(hiddenVertices[0]);
			while (hiddenEdges.Count > 0) UnhideEdge(hiddenEdges[0]);
			return true;
		}

		public int HiddenVertexCount => hiddenVertices.Count;

		public int HiddenEdgeCount => hiddenEdges.Count;
