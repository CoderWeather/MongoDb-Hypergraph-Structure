...
namespace TestConsole
{
	internal static class Program
	{
		...
		private static async Task ParseCollectionAsync(IMongoCollection<BsonDocument> collection)
		{
			...
			foreach (var vertex in hg.VerticesList)
			{
			...
			sw.WriteLine("Edges:");
			foreach (var edge in hg.EdgesList)
			{
			...
			sw.WriteLine("Vertices count: " + hg.VerticesList.Count);
			sw.WriteLine("Edges count: " + hg.EdgesList.Count);
